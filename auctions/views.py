    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            context['is_favorited'] = Favorite.objects.filter(
                ...
                sale__packages__products=product
            ).exists()
...
def place_bid(request, pk):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            error_message = e.message_dict.get('amount', ["Invalid bid"])[0]
...
def toggle_favorite(request, pk):
    """Add/remove sale from favorites"""
    ...
    if created:
        messages.success(request, "Added to watched sales.")
    else:
        favorite.delete()
        messages.success(request, "Removed from watched sales.")
...
class SaleDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Check if user is watching this sale
        if self.request.user.is_authenticated:
            context['is_watching'] = sale.watchers.filter(id=self.request.user.id).exists()
